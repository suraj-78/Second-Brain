...
const HomePage = ()=>{
  ...
  if(dataShow === "All"){
    show = loading ? (
      ...
      return <Card key={idx} _id={item._id} icon={item.contentType} tag={item.tag} title={item.title} link={item.link} reload={()=> setReloadData(!reloadData)}/>
    ...
    show = loading ? (
      <div className="text-2xl font-semibold">Loading...</div>
    ) : ( twitterData.length > 0 ? twitterData.map((item: any, idx: number)=>{
      return <Card key={idx} _id={item._id} icon={item.contentType} tag={item.tag} title={item.title} link={item.link} reload={()=> setReloadData(!reloadData)}/>
    }) : <div className="text-2xl font-semibold">You do not have any Content</div>
    )
  }
  ...
  return <div className="flex">
   <SideNavbar setData={setData} setYTData={setYTData} setNitionData={setNitionData} setTwitterData={setTwitterData} data1={data1} setDataShow={setDataShow}/>
